...
public class DownloadDirectoryProvider {
        ...
        protected ArrayList<DirectoryOption> doInBackground() {
            ...
            if (defaultDirectory == null) {
                dirs.add(new DirectoryOption(
                        null, 0, 0, DownloadLocationDirectoryType.ERROR));
                return dirs;
            }

            DirectoryOption defaultOption = toDirectoryOption(
                    defaultDirectory, DownloadLocationDirectoryType.DEFAULT);
            dirs.add(defaultOption);
            recordDirectoryType(DownloadLocationDirectoryType.DEFAULT);
            ...
            List<File> secondaryDirs = VERSION.SDK_INT > VERSION_CODES.Q
            ...
            for (File file : secondaryDirs) {
                ...
                dirs.add(toDirectoryOption(
                        file, DownloadLocationDirectoryType.ADDITIONAL));
                hasAddtionalDirectory = true;
            }

            if (hasAddtionalDirectory) {
                recordDirectoryType(DownloadLocationDirectoryType.ADDITIONAL);
            }
    ...
    private void recordDirectoryType(@DownloadLocationDirectoryType int type) {
        RecordHistogram.recordEnumeratedHistogram("MobileDownload.Location.DirectoryType", type,
                DownloadLocationDirectoryType.NUM_ENTRIES);
    }
}
